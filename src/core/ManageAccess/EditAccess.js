...
import { changeUserRole, cleanUpManageAccess, updateRole, updateUser,
    getAllRoles, getUserRole, showLoader } from 'app/redux/actions/manageAccessActions';
...
function EditAccess() {
    ...
    const changeEmployee = (e) => {
        ...
        let valid = false;
        if(e.target.value != '' && manageAccessState.editAccess.currentRole != '') valid = true
        setValid(valid)
        if(e.target.value != ''){
            dispatch(showLoader('editAccess'))
    ...
    const changeRole = (e) => {
        let valid = false;
        if(e.target.value != '' && manageAccessState.editAccess.user != '') valid = true
        setValid(valid)
        dispatch(updateRole(e.target.value))
    }
    const addRoleToUser = () => {
        if((manageAccessState.editAccess.currentRole != '') &&
         (manageAccessState.editAccess.user != ''))
        {
            dispatch(showLoader('editAccess'))
            dispatch(changeUserRole(manageAccessState.editAccess.user, 
    ...
    return (
        ...
        <div className="mt-4 col-md-4 col-sm-8">
            ...
            <div className="form-group">
                ...
                <select className="form-control" onChange={changeEmployee} 
                value={manageAccessState.editAccess.user}>
                    ...
                    commconState.allAdminData.allAdminUsers.map(user => (
                        <option key={user.id} value={user.emailId}>{user.name}</option>
                    ))
                    : null
                    }
                </select>
            ...
            <div className="form-group">
                ...
                <select className="form-control" onChange={changeRole} 
        ...
        {manageAccessState.editAccess.loading ? (<div className="mt-5 align-self-start" >
           <Loader height="33px" />
        </div>) : null}
        </div>
        <div className="row">
            {manageAccessState.editAccess.error ? (<div className="mt-4 col" >
            <div className="alert alert-danger" role="alert">
                {manageAccessState.editAccess.errorMessage}
            </div>
            </div>) : null}
            {manageAccessState.editAccess.success ? (<div className="mt-4 col" >
