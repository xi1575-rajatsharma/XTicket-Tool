...
export const searchUserAction = (label, value) => (dispatch) => {
    XenieApi.get(
      `${exportUrl + configs.searchUser}?label=${label}&value=${value}`      
        ...
        if(resp.data.result && resp.data.result.users && resp.data.result.users.length){
            dispatch({
          ...
          dispatch({ type: types.SHOW_ERROR, error:{response: { data: 
...
export const assignDepttoUser = (emailId, departmentId) => (dispatch) => {
    XenieApi.post(
      `${exportUrl + configs.assignDepttoUser}?emailId=${emailId}&departmentId=${departmentId}`     
    ).then(
      (response) => {
        dispatch({
...
export const getUserRole = (email) => (dispatch) => {
    XenieApi.get(
      `${exportUrl + configs.getUserRole}?email=${email}`      
    ).then(
      (response) => {
        dispatch({
...
export const getAllRoles = () => (dispatch) => {
    XenieApi.get(`${exportUrl + configs.getAllRoles}`      
    ).then((response) => {
        dispatch({
